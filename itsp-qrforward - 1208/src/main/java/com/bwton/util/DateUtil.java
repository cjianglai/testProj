...
public class DateUtil {
    ...
    public static int getIntervalDays(Date date, Date otherDate) {
        ...
        long diff = (endTime - startTime) / (1000 * 3600 * 24);
        return (int) diff;
    }
     ...
     * @return
     */
    public static long getIntervalSecondByDate(Date startDate, Date endDate) {
        long interval = -1;
        if (startDate == null || endDate == null) {
            return interval;
        }
    ...
    public static Date formatDate(String date)
    {
        ...
        DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//创建日期转换对象HH:mm:ss为时分秒，年月日为yyyy-MM-dd
        try {
            Date dt = df.parse(date);//将字符串转换为date类型
